...
#include "../../../../../build/openpilotgcs-synthetics/version_info.h"
    ...
    outputFileName(outputKmlFileName)
{
    ...
    // Get the UAVObjects
    airspeedState = AirspeedState::GetInstance(kmlUAVObjectManager);
    attitudeState = AttitudeState::GetInstance(kmlUAVObjectManager);
    gpsPosition = GPSPositionSensor::GetInstance(kmlUAVObjectManager);
    homeLocation = HomeLocation::GetInstance(kmlUAVObjectManager);
    positionState = PositionState::GetInstance(kmlUAVObjectManager);
    velocityState = VelocityState::GetInstance(kmlUAVObjectManager);
    ...
    // KML placemark.
    connect(positionState, SIGNAL(objectUpdated(UAVObject *)), this, SLOT(positionStateUpdated(UAVObject *)), Qt::DirectConnection);
...
bool KmlExport::open()
{
    ...
    QString gitHash = QString::fromLatin1(GCS_REVISION_STR);
    QString uavoHash = QString::fromLatin1(UAVO_HASH_STR).replace("\"{ ", "").replace(" }\"", "").replace(",", "").replace("0x", ""); // See comment above for necessity for string replacements
...
PlacemarkPtr KmlExport::createTimespanPlacemark(const LLAVCoordinates &timestampPoint, quint32 lastPlacemarkTime, quint32 newPlacemarkTime)
{
    ...
    AirspeedState::DataFields airspeedStateData = airspeedState->getData();
    IconStylePtr iconStyle = factory->CreateIconStyle();
    iconStyle->set_color(mapVelocity2Color(airspeedStateData.CalibratedAirspeed));
    iconStyle->set_heading(attitudeStateData.Yaw + 180); //Adding 180 degrees because the arrow art points down, i.e. south.
...
/**
 * @brief KmlExport::positionStateUpdated Triggers on PositionState UAVO
 * update. Converts position to latitude-longitude-altitude and then
 * creates new placemarks.
 * @param obj Unused
 */
void KmlExport::positionStateUpdated(UAVObject *obj)
{
    Q_UNUSED(obj);
    ...
    if (gpsPositionData.Status != GPSPositionSensor::STATUS_FIX2D &&
            gpsPositionData.Status != GPSPositionSensor::STATUS_FIX3D)
        return;

    AirspeedState::DataFields airspeedStateData = airspeedState->getData();
    PositionState::DataFields positionStateData = positionState->getData();
    VelocityState::DataFields velocityStateData = velocityState->getData();
    ...
    newPoint.groundspeed = sqrt(velocityStateData.North*velocityStateData.North + velocityStateData.East*velocityStateData.East);

    // Update UAV info string
    informationString.clear();
    informationString.append(QString("Latitude: %1 deg\nLongitude: %2 deg\nAltitude: %3 m\nAirspeed: %4 m/s\nGroundspeed: %5 m/s\n").arg(newPoint.latitude)
                             .arg(newPoint.longitude).arg(newPoint.altitude).arg(airspeedStateData.CalibratedAirspeed).arg(newPoint.groundspeed));
